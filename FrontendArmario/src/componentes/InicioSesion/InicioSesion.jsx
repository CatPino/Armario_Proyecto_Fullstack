  ...
  const validarCampo = (id, valor) => {
    ...
    if (id === "email") {
      if (!valor || !emailPermitido.test(valor.trim())) {
        ...
        if (err) err.textContent = "Ingresa un correo válido (@duoc.cl, @profesor.duoc.cl o @gmail.com)";
        return false;
      } else {
    ...
    if (id === "password") {
      if (!valor || valor.trim().length < 8) {
        input.classList.add("is-invalid");
        if (err) err.textContent = "La contraseña debe tener al menos 8 caracteres.";
        return false;
      } else {
        input.classList.add("is-valid");
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!respuesta.ok) {
        setMensaje({
          tipo: "error",
          texto: data || "⚠️ Credenciales incorrectas.",
        });
      } else {
        ...
        <div className="card card-login">
          <form onSubmit={handleLogin} id="loginForm" method="post" style={{ marginTop: "15px" }}>
            ...
            <div className="mb-3">
              ...
              <input type="email" className="form-control" id="email" name="email" placeholder="tucorreo@ejemplo.com" value={email}
                onChange={(e) => setEmail(e.target.value)} onBlur={(e) => validarCampo("email", e.target.value)}/>
            ...
            <div className="mb-3">
              ...
              <input type="password" className="form-control" id="password" name="password" placeholder="Contraseña"
                value={password} onChange={(e) => setPassword(e.target.value)} onBlur={(e) => validarCampo("password", e.target.value)}/>
